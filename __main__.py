...
if __name__ == '__main__':
    ...
    # Create the track to Niels office 
    height = 5.0
    start = compute_traj([(0.0, 0.0), (0.2, 0.0), (0.5, 0.0)])
    straight = compute_traj([(1.5, 1.0), (1.5, height/2), (1.5, height-1)])
    end = compute_traj([(0.5, height), (0.2, height), (-2.0, height)])
    
    goal = start
    goal = np.vstack((goal, straight))
    goal = np.vstack((goal, end))
    ...
    np.savetxt("Results/Robot.csv", trajectory, delimiter=",")
    ...
    print("Elapsed time: ", round(end - start, 2))
    #np.savetxt("Results/Simulering.csv", trajectory, delimiter=",")
    ...
    plt.legend()
    plt.gca().set_position([0, 0, 1, 1])
    plt.savefig("Results/Simulering.svg")
    plt.show()
    # '''
