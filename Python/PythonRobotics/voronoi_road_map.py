...
def main():
    ...
    ori=cv2.imread('ICP/Corrected_optimized_no_axises.jpg')
    ...
    """
    if show_animation:  # pragma: no cover
        plt.plot(ox, oy, ".k") # Plot Edges
        plt.plot(sx, sy, "^r") # Plot start point
        plt.plot(gx, gy, "^c") # Plot goal point
        plt.grid(True) # Setup grid
        
        #plt.axis("equal") # Change axis length
        plt.show()
    
    #rx, ry = 
    VoronoiRoadMapPlanner().planning(sx, sy, gx, gy, ox, oy,
                                              robot_size)
